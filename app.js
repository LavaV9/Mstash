...
app.use((req, res, next) => {
  ...
  res.locals.searchQuery = req.query.q || ''; // Ensure searchQuery is available
  next();
});
...
app.use((err, req, res, next) => {
  const searchQuery = req.query.q || ''; // Ensure searchQuery is available even in errors
  if (!err.status) {
    err.status = 500;
  ...
  res.render("error", {
    error: err,
    searchQuery: searchQuery // Pass searchQuery to the error view
  });
});
